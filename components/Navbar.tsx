  ...
  return (
    <header className="site-nav">
      <nav className="nav-inner" aria-label="Primary navigation">
        ...
        <a className="nav-cta" href={`mailto:${siteConfig.contact.email}`}>Contact Shivam <ArrowUpRight className="icon" /></a>
      ...
      <AnimatePresence>
        {open && (
          <motion.div className="mobile-menu" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            ...
            <a href={`mailto:${siteConfig.contact.email}`} onClick={() => setOpen(false)}>Contact Shivam <ArrowUpRight className="icon" /></a>
